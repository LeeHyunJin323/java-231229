...
//		System.out.println(calendar); ??
		...
		System.out.println("생일의 년도를 쓰세요.");
		birthdayYear = sc.nextInt();
		System.out.println("생일의 달을 쓰세요.");
		birthdayMonth = sc.nextInt();
		System.out.println("생일의 날짜를 쓰세요.");
		birthdayDay = sc.nextInt();
		birthday.set(Calendar.YEAR ,birthdayYear);
		birthday.set(Calendar.MONTH ,birthdayMonth-1);
		// 캘린더에서 받은 month는 -1이기 때문에 스캐너로 받은 month값을 -1 해줌
		
		birthday.set(Calendar.DAY_OF_MONTH ,birthdayDay);
		// birthday의 날짜를 현재의 날짜를 받고 스캐너로 입력받은 날짜로
		// 재할당 받음
		

		//today에는 현재 날짜
		// birthday에는 내 생일 
		// (birthday.before(today)) 조건식으로 현재 생일이 지났는지 판단
		//앞의 날짜가(birthday) 뒤에날짜(today)보다 지났는가?
		if(birthday.before(today)) {
			// 이미 지났다면 내년으로 설정
			//만약 지났다면 year값 + 1을하여 내년으로 설정
...
//		1시간 = 60분
//		1일 = 24시간
		long difference = (birthday.getTimeInMillis() - today.getTimeInMillis()) // 생일과 오늘 날짜를 초로 바꾸어서 계산 후
				// 다시  (1000* 60 *60 *24)를 이용하여 날짜로 계산
				/(1000* 60 *60 *24);
