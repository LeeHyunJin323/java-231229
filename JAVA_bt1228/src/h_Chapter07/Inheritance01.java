...
class Parent  {} // : 기존 클래스
class Child extends Parent {} // " 세로 작성하려는 클래스
...
// 자식 안에 부모가 있음 -> 부모에게 쌍커풀을 상속 받고 + 내가 가진것들이기 때문에 +@임

// Dog 자식 클래스는 Animal 부모 클래스의 상속을 받음
// : 자식 클래스는 부모 클래스의 모든 멤버를 상속 받기 때문에
// : , 자식 클래스의 멤버(속성, 메서드) 개수는 부모클래스보다 항상 같거나 많음
...
class Tv{
	...
	int channel;
	//boolean의 기본값은 true
	void power() {power =! power;} // -> power가 true 였다면 false , false 였다면 true가 됨
...
public class Inheritance01 {
	...
	public static void main(String[] args) {
		...
		// >> 따로 부모 클래스의 인스턴스를 생성하지 안호고도
		//      , 부모 클래스의 멤버들을 사용 가능
		
		stv.channel = 10; // 부모 클래스로 부터 상속받은 멤버(변수)
